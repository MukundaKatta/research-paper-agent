"""FastAPI application for research-paper-agent."""
...
app = FastAPI(title="research-paper-agent", version="0.1.0", description="research-paper-agent API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "research-paper-agent", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "research-paper-agent", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "research-paper-agent", "version": "0.1.0", "ready": True,
